...
9. [改修スケジュールと影響範囲マトリクス](#9-改修スケジュールと影響範囲マトリクス)
...
| killDenormal 全呼出箇所 | **確定**: **20箇所**（8ファイル） | grep結果 |
...
#if defined(__AVX2__)
/// AVX2 版バッチ isBadSample: 4要素を1命令でチェック
inline bool isBadSampleV(__m256d v) noexcept
{
    // NaN 検出: _CMP_UNORD_Q は v のいずれかが NaN の場合 true
    const __m256d vNanMask = _mm256_cmp_pd(v, v, _CMP_UNORD_Q);
    // Inf/巨大値検出: 絶対値が 1e20 を超える
...
// 【旧コード】
const double s0 = history[idx0];
// ... (4回スカラーロード + 4回 isBadSample)
const __m256d vSamples = _mm256_set_pd(s3, s2, s1, s0);

// 【新コード】1回の SIMD ロード + 1回の SIMD チェック
...
// 仮に kSpreadMaxDeltaBark = 24.0 とすると、0.01 Bark 刻みで 4801 エントリ

static constexpr int kSpreadTableBins = 4801;  // -24.0 ~ +24.0 @ 0.01 step
...
| # | 事項 | 状況 | 調査方法 |
|---|---|---|---|
| U-1 | **`kSpreadMaxDeltaBark` の値** | 未確認 | MklFftEvaluator.h 冒頭の定数定義を確認 |
| U-2 | **spreadingFunctionAnnexD の constexpr 対応** | C++20 constexpr で `std::sqrt` が constexpr 評価可能か要確認 | MSVC の C++20 constexpr sqrt 対応状況 |
| U-3 | **generationIntervalSeconds の典型的な値** | ユーザー設定依存 | 設定ファイル／UIのデフォルト値確認 |
| U-4 | **ISR traceBuffer サイズ (4096) の妥当性** | 最大 122s / ~2μs間隔 = 61Mエントリまで可能性 | 実運用での最大 transition 頻度を推定 |
