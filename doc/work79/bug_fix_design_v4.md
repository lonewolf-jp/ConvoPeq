# ConvoPeq 未修正バグ改修設計書（v4 — 最終版）

**作成日**: 2026-07-21
**レビュー反映**: 2026-07-21（v3 レビュー → v4 反映：最終 3 点修正）
...
│  │    fb = sanitizeFinite(fb)  ← NaN/Inf 除去（追加）     │   │
│  └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  quantize()                                            │   │
│  │    v = sanitizeFinite(v)  ← NaN/Inf 除去（追加）        │   │
│  │    if (v < minV) v = minV;                             │   │
...
1. **DSP Numeric Policy 層**: `sanitizeFinite()` を定義（汎用ヘルパー）
2. **NoiseShaper 入口層**: `processSample()` の fb に `sanitizeFinite()` を適用
3. **量子化層**: `quantize()` の最前部に `sanitizeFinite()` を適用
4. **出口層**: 出力バッファへの書き込み前に最終チェック（既存の `saturateAVX2` で一部保護）
...
- パフォーマンス影響: 非常に軽量なビット判定処理。浮動小数点の指数部のみを確認する軽量処理であり、通常の DSP 負荷に対して十分小さい
...
現在のコードは既に `convo::aligned_make_unique` を使用しており、`std::unique_ptr` で所有権管理されている。重要なのは **`release()` に到達するまで RAII によって保護されている** ことです。
...
try
{
    ...
    // ★ nuc0, nuc1 は unique_ptr で RAII 保護されている。
    //   SetImpulse() 中で例外が発生しても、スコープ離脱時に自動破棄される。
        ...
        nuc1->SetImpulse(irData[1], irDataLength, knownBlockSize, scale, enableDirectHead, filterSpec))
    {
        ...
        // ★ release() 以降は手動管理になるが、この時点では SetImpulse が成功済み
    ...
    // SetImpulse 失敗: nuc0, nuc1 はスコープ離脱時に自動破棄（RAII）
}
...
- `release()` は所有権放棄であり、それ自体が安全を保証するものではない
- 重要なのは **`release()` に到達するまで RAII によって保護されている** こと
- `SetImpulse()` 中で例外が発生しても、`unique_ptr` のデストラクタが自動的にリソースを解放する
- `catch` で明示的にクリーンアップする必要はない（RAII が担う）
...
**問題**: `init` 失敗時の cleanup で `storedFilterSpec` / `hasStoredFilterSpec` がリセットされない。`init` が再呼び出しされた場合、前回の `storedFilterSpec` が使用される可能性がある。
...
**問題**: `init` 失敗時の cleanup で `callQuantumSamples` / `storedSampleRate` / `storedKnownBlockSize` / `storedScale` / `storedDirectHeadEnabled` がリセットされない。
...
**問題**: `delayWritePos` は非アトミックのメンバ変数。`processBypassWithLatencyCompensation` は Audio Thread から呼ばれるが、`reset()` も `delayWritePos` を書き込む。`reset()` が Audio Thread 以外から呼ばれた場合、データレースが発生する。

**修正案**: `delayWritePos` を `std::atomic<int>` に変更するか、`reset()` が Audio Thread からのみ呼ばれることを保証する。
...
**問題**: `Get()` が `numSamples` より大きい値を返した場合、バッファオーバーフローが発生する。
