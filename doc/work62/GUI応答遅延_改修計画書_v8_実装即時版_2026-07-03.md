# ConvoPeq GUI応答遅延 — 改修計画書 v8（実装即時版）

**作成日**: 2026-07-03 (v8.0)
**位置づけ**: doc/work62/ 全10文書の集大成。本計画書のみで実装可能。
...
void AudioEngine::timerCallback()
{
    ...
    // ★ Other = timerCallback の残余時間
    //    CB_HIST と DiagDrain 以外のすべての処理（VERIFY, MEM, CBSUMMARY, WORLD,
    //    Learning, HealthMonitor, Overflow drain, 集計処理自体, そして diagLog も含む）
    //    をまとめて「Other」として計測する。
    //    名前の通り「残差」であり、ScopedBlockTimer では計測できない。
...
| 日付 | 版 | 変更内容 |
|------|-----|---------|
| 2026-07-03 | v8.0 | ユーザーレビュー v7 の指摘を反映。diagSink を nullptr から fileSink で初期化（実行時分岐排除）。「Other = timerCallback 残余時間」と明記。pushWithWriter の bool 戻り値を確認済みとして追記。v7 の設計は全て維持。 |
| 2026-07-03 | v7.0 | ユーザーレビュー v6 の指摘を反映。Step 0 Other 計測を全面的に再設計。 |
