...
| 区分 | 項目 | 状態 |
|------|------|------|
| バグ修正 | BUG-014（MmcssPolicy enum atomic 化） | ✅ 実装済み（`currentMmcssPolicy_` atomic + static_assert） |
| バグ修正 | BUG-015/027（RetireQuarantineStore 新設） | ✅ 実装済み（`RetireQuarantineStore.h` 新設 / Router API `quarantineRetire()` / drain 配線） |
| バグ修正 | BUG-028（CrossflareRuntime RT 専有化） | ✅ 実装済み（`start()` の `gain_.setCurrentAndTargetValue(0.0)` 削除 / `complete()` の `dryScaleTarget_`/`startDelayBlocks_`/`dryHoldSamples_` atomic publish / `crossfadeGeneration_`） |
| バグ修正 | BUG-060 / BUG-061 | ✅ 実装済み（二次レビューで訂正） |
| FUTURE-3 | recovery-world build の Builder Loop 接続（`buildSource`（RuntimeBuildSnapshot 値コピー）を payload に内包） | ✅ 実装済み（`RecoveryIntent::buildSource` 追加 / `submitRecoveryRequest(handle, buildSource)` / `RebuildDispatch.cpp:911` 消費 / `recoveryPending` 起床配線） |
| FUTURE-9 | SHUTDOWN-7: shutdown 順序確定（完了条件への組込みは撤回） | ✅ 実装済み（`stopRebuildThread(join)` → `waitForDrain` 順序 + jassert 契約） |
| FUTURE-10 | 共通 Intent Queue MPSC 化 + Observe 統合 | ✅ 実装済み（`MpscBoundedRing.h` 新設 / `intentQueue_` MPSC 化 / `submitObserve` 統合 / `quarantineFallbackQueue_` 追加） |
| FUTURE-10 | `RecoveryIntentHandler::handle` 実装（enqueue-only + Builder Work Queue 転送） | ✅ 実装済み（`ProcessIntent.cpp:99-108` — 循環排除済み） |
| 検証 | Shutdown Pipeline 検証（SHUTDOWN-1〜7 と Intent Queue の対応） | ☐ 未着手 |
| FUTURE-5 | MemoryPool化 | ☐ 未着手 |
| FUTURE-6 | Handle Table 完全移行 | ☐ 未着手（※ 部分着手: `runtimeDSPHandleMap_` → `DSPHandleTable` 線形走査化） |
...
> **六次レビュー実装照合（2026-08-08 work88 適用後 — 実装済み項目の確定 + 新規発見）:**
>
> 章0 の判定表が実装前の状態のままだったため、**実装後のコードを再照合**して以下の通り確定した。
>
> | 項目 | 実装状態 | 根拠コード |
> |------|----------|-----------|
> | BUG-014（MmcssPolicy enum atomic） | ✅ 実装済み | `AudioEngine.h:2347-2383` — `enum class MmcssPolicy : uint8_t` + `std::atomic<MmcssPolicy> currentMmcssPolicy_` + static_assert 2 本（trivially_copyable / is_always_lock_free）+ `setAudioDeviceTypeName` 文字列比較 + `publishAtomic` |
> | BUG-028（CrossflareRuntime RT 専有化） | ✅ 実装済み | `CrossfadeRuntime.h:40-46` — `start()` の `gain_.setCurrentAndTargetValue(0.0)` **削除済み**。`complete()`（:106-121）で `dryScaleTarget_`/`startDelayBlocks_`/`dryHoldSamples_` を **atomic publish**。`crossfadeGeneration_` 追加（:196）。`gain_.setCurrentAndTargetValue(1.0)` は `reset()`（:138-139, RT 停止後）のみ |
| BUG-015/027（RetireQuarantineStore） | ✅ 実装済み | `RetireQuarantineStore.h` 新設（`std::array` + index 配置 = allocation-free / `kMaxQuarantinedEntries=512` / `drain(isOlderFn)`）。`ISRRetireRouter.h:198` 配下に単一配置。`quarantineRetire()`（cpp:229）。`enqueueWithRetry` 失敗時に `m_retireQuarantine.quarantine()`（cpp:190）。`tryReclaim` → `drainQuarantineStore()`（cpp:211,217）。`SnapshotCoordinator` も `quarantineRetireSink` 経由で移送 |
| FUTURE-3（Recovery buildSource） | ✅ 実装済み | `ISRRuntimePublicationCoordinator.h:160-172` — `RecoveryIntent::buildSource`（`RuntimeBuildSnapshot` 値コピー）追加 + `static_assert(trivially_copyable_v<RuntimeBuildSnapshot>)`。`submitRecoveryRequest(handle, buildSource)`（cpp:643-657）。`RebuildDispatch.cpp:891-963` — `popRecoveryRequest` → `runtimeBuilder.build(buildSource.buildInput, convolverSnapshot)` → `enqueuePublicationIntentForRuntimeCommit` |
| FUTURE-10（MpscBoundedRing） | ✅ 実装済み | `MpscBoundedRing.h` 新設（Vyukov bounded MPSC / CAS slot 予約 / seq release）。`ISRRuntimePublicationCoordinator.h:434` — `intentQueue_` が `MpscBoundedRing<Intent,4096>`。`quarantineFallbackQueue_`（:441-442）追加 |
| FUTURE-10（Observe 統合） | ✅ 実装済み | `submitObserve`（cpp:531-575）— `intentQueue_.push` を primary に、full 時 `observeDeferredRing_` 退避。`processIntent` で共通 `intentQueue_` を drain（専用 SPSC ring の while-pop 廃止） |
| FUTURE-10（RecoveryIntentHandler） | ✅ 実装済み | `ISRRuntimePublicationCoordinator_ProcessIntent.cpp:99-108` — no-op から **enqueue-only + Builder Work Queue 転送**に実装（`ctx.engine.submitRecoveryIntent(...)`）。循環排除（pop 元と書込先が別キュー） |
| FUTURE-10（Recovery 起床配線） | ✅ 実装済み | `AudioEngine.h:4207-4219` — `submitRecoveryIntent` が `recoveryPending=true` + `rebuildCV.notify_all()`。`RebuildDispatch.cpp:815` で `recoveryPending` により起床 |
| FUTURE-10（Quarantine drop 禁止） | ✅ 実装済み | `submitQuarantine`（cpp:676-718）— `intentQueue_` full → `quarantineFallbackQueue_` 退避 → 両方 full 時のみ `quarantineFallbackDropCount_` increment（drop カウンタ残存 + 診断）。静かな破棄なし |
| FUTURE-6（HandleTable 部分） | 🔄 部分着手 | `DSPLifetimeManager.cpp:70-76` — `runtimeDSPHandleMap_.findAndEraseByHandle`（O(n) 線形走査・固定 512 上限）に置換済み |
| SHUTDOWN-7（順序） | ✅ 実装済み | `ReleaseResources.cpp:75→188→189→190→191→430` の順序不変（join → waitForDrain）。`ISRShutdown.cpp:108` transitionTo の順序検証 |
>
> **六次レビューで新規発見した問題（3件 — 実装のバグ or 計画書との乖離）:**
>
> 1. **🔴 P1 — `RetireQuarantineStore::drain()` / `drainAllUnsafe()` が mutex 保持中に deleter を呼ぶ**: 三次レビュー指示（「lock → safe entries 抽出 → unlock → deleter」）に違反。deleter が再entrant（別の retire/quarantine を呼ぶ）とデッドロック。**2026-08-08 六次レビューにて修正**（deleter 呼び出しを unlock 後に移動 — `RetireQuarantineStore.h:98-130`）。この修正は計画書の設計契約（lock 中 deleter 禁止）に準拠。
> 2. **🔴 P1 — `retireDSPHandleForRuntime` の `shutdownReclaim` が残存**: `AudioEngine.h:4198` に `dspHandleRuntime_.shutdownReclaim(handle)` がまだ存在（計画書の「shutdownReclaim transitional 措置は削除し requestReclaim に一本化」が未実施）。**2026-08-08 六次レビューにて修正**: `shutdownReclaim` を撤廃し `requestReclaim` に一本化。`retireDSPHandleForRuntime`（`AudioEngine.h:4193-4222`）は epoch 安全確認（`currentEpoch() < minReaderEpoch()`）を行い、(a) 安全なら `runtimePublicationBridge_.requestReclaim(...)` を即時実行、(b) 不安全なら `dspHandleRuntime_.retire(handle)` + `pendingReclaimHandles_` に登録 → `drainDeferredRetireQueues`（定期的に minReaderEpoch が進んだ後に実行）で再試行（slot リーク防止）。**注意: `requestReclaim` 単発呼び出しでは、RT Reader が古い epoch を参照中の間 reclaim を遅延するが再試行されないため、保留リスト方式で確実に収束させる。** 再試行時は `dspHandleRuntime_.isRetired(handle)`（新設 accessor）でガードし、`quarantineSlot` が後から Quarantined に遷移した handle を Retired に上書きしない（`ISRDSPHandle.h:152-160`）。`retire(handle)` は requestReclaim 内部でも冪等実行される。DSPCore* の物理削除は呼び出し元（`DSPLifetimeManager::retire`/`retireByHandle`）が `enqueueWithRetry` で行い、失敗時は RetireQuarantineStore へ移送済み（二重移送なし）。**`shutdownReclaim` の残存使用は shutdown 専用に限定確認済み**: `CacheMap::~CacheMap`（AudioEngine.h:2025, shutdownPhase>=Destroy 時のみ）と `releaseResources()` の VerifyDrained（ReleaseResources.cpp:415,420, Audio Thread 停止後）。runtime 経路からの使用はゼロ。
> 3. **🟡 P2 — 旧 SPSC Observe ring（`observeIntentQueue_`/`observeFallbackQueue_`）がデッドコードとして残存**: `ISRRuntimePublicationCoordinator.h:406-411` にまだ宣言があり、`LockFreeRingBuffer` 分のメモリ（ObserveIntent×1024+2048）を確保。`submitObserve` は `intentQueue_` に push するようになったため、旧 ring は **push も pop もされない**。**2026-08-08 六次レビューにて削除済み**（`ISRRuntimePublicationCoordinator.h` — 宣言と capacity 定数を除去。`observeDeferredRing_` は overflow 退避先として使用継続）。
> 4. **🟡 P2 — `quarantineFallbackDropCount` の監視配線が未確認**: `submitQuarantine` が drop カウンタを increment するが、**これを読む getter（`quarantineFallbackDropCount()`）の呼び出し元が Timer / HealthMonitor に見当たらない**。**2026-08-08 六次レビューにて配線**: (a) `RuntimeBackpressureTelemetry` に `quarantineFallbackDropCount` フィールドを追加（`AudioEngine.h`）、`getRuntimeBackpressureTelemetry()` で `runtimePublicationBridge_.quarantineFallbackDropCount()` を公開。(b) `AudioEngine.Retire.cpp` の `drainDeferredRetireQueues` の overflow→throttle 判定に統合 — drop が発生した場合 `effectiveLevel = 3`（Critical 相当）として `injectBackpressureSignal` 経由で RuntimeHealthMonitor 評価 → 停止判断。
> 5. **🔴 P1 — `retireByHandle`（Observe 経路）が `requestReclaim` を呼ばず slot リーク**: `DSPLifetimeManager::retireByHandle`（DSPLifetimeManager.cpp:84）は `engine_.dspHandleRuntime_.retire(handle)` のみで `requestReclaim` を呼ばない。ObserveIntentHandler / drainObserveDeferred から呼ばれるため、Observe 経路の handle が Retired のまま Reclaimed にならず、**256 slot が枯渇するリスク**。**2026-08-08 六次レビューにて修正**: `engine_.requestReclaimHandle(handle)`（新設共通ヘルパー — epoch 安全なら requestReclaim、不安全なら保留リスト）を `retireByHandle` に追加。`retireDSPHandleForRuntime` も同ヘルパーに共通化。
> 6. **🔴 P1 — Recovery 発行経路（quarantine → Recovery）が未配線 = Recovery 機能はデッドコード**: 計画書（:654-656）は「quarantine 検出時（`QuarantineIntentHandler` / `QuarantineService::executeQuarantine`）に、quarantined DSP の build spec を引当してから Recovery Intent を発行する」と設計。しかし実装では `QuarantineIntentHandler::handle`（ProcessIntent.cpp:73-86）は `executeQuarantine` のみで **Recovery Intent を発行しない**。`RecoveryIntentHandler::handle`（enqueue-only, ProcessIntent.cpp:103-109）は実装済みだが、**トリガー（Recovery Intent の push 経路）が存在しない**。`submitRecoveryRequest` の実呼び出し元は `submitRecoveryIntent`（AudioEngine.h:4245）のみで、その唯一の呼び出し元は `RecoveryIntentHandler::handle`。つまり **Recovery は決して発行されない**。**2026-08-08 六次レビューにて修正**: (a) AudioEngine に「現在の publish 構成 snapshot」`currentBuildSnapshot_`（mutex 保護）を追加、`enqueuePublicationIntentForRuntimeCommit` で更新（AudioEngine.Commit.cpp）。(b) `getCurrentBuildSnapshotForRecovery()` を追加。(c) `QuarantineIntentHandler::handle` が `executeQuarantine` 後に `getCurrentBuildSnapshotForRecovery()` → `submitRecoveryIntent(handle, buildSource)` を呼ぶ（ProcessIntent.cpp — Recovery 発行の primary 経路）。Recovery フロー: quarantine → submitRecoveryIntent → submitRecoveryRequest → recoveryIntentQueue_（Builder Work Queue）→ Builder Loop が popRecoveryRequest で消費（RebuildDispatch.cpp:911）。`RecoveryIntentHandler`（intentQueue_ 経由）は将来の拡張用に残置。buildSource は現在の構成 snapshot（Recovery = 現在のユーザー構成を再 build + quarantined 除外, 計画書:608-610）。
> 7. **🔴 P1 — `submitRecoveryRequest` が `recoveryIntentQueue_.push()` 失敗を無視（INV-5 違反）**: `recoveryIntentQueue_`（LockFreeRingBuffer<RecoveryIntent,256>, SPSC）が full のとき push が失敗し、Recovery Intent が静かに drop。さらに `pendingIntentCount_ + 1` が push 失敗時も実行され、`popRecoveryRequest` が drop 分を -1 しないため `isFullyDrained` が false のまま = **shutdown ハング**の可能性。**2026-08-08 六次レビューにて修正**: `submitRecoveryRequest`（ISRRuntimePublicationCoordinator.cpp:643-669）で push 戻り値をチェック — 成功時のみ `pendingIntentCount_ + 1`、失敗時は `recoveryIntentDropCount_`（新設 atomic）に記録し静かな破棄を禁止。`recoveryIntentDropCount()` getter（public）+ `prevRecoveryIntentDropSnapshot_` を追加し、`drainDeferredRetireQueues` の overflow→throttle 判定と `RuntimeBackpressureTelemetry` に統合（`recoveryDropActive` → `effectiveLevel = 3` Critical 相当）。
> 8. **🟡 P2 — `RecoveryIntentHandler` は dead code（ドキュメントと実装の乖離）**: `RecoveryIntentHandler::handle`（ProcessIntent.cpp:117-123）は `intentQueue_` から pop した Recovery Intent を転送するが、`submitRecoveryRequest` は `intentQueue_` ではなく `recoveryIntentQueue_` に直接 push するため、**`intentQueue_` に Recovery Intent を push する経路が存在せず dead code**。二重発行は発生しない（Recovery 発行は `QuarantineIntentHandler` → `submitRecoveryIntent` の primary 経路のみ）。**2026-08-08 六次レビューにてコメント修正**: `RecoveryIntentHandler` を「将来の拡張経路（intentQueue_ 経由で Recovery を流す場合）に備えた転送ハンドラ」と明記。機能削除はせず、将来拡張に備えて残置。
> 9. **🟡 P2 — per-type admission policy は統一機構として未実装（各 submit 関数に分散実装）**: 計画書（FUTURE-10, 三次レビュー）は「per-type admission policy 表（Observe=coalesce、Publish/Quarantine/Recovery=drop 禁止）」を要求。検証結果、**統一された `AdmissionPolicy` クラス/エンジンは存在せず**、機能は各 submit 関数に分散実装されている: `submitObserve`（intentQueue_ → observeDeferredRing_ → drop カウンタ, 条件付き drop）、`submitQuarantine`（intentQueue_ → quarantineFallbackQueue_ → drop カウンタ + Critical 昇格）、`submitRecoveryRequest`（recoveryIntentQueue_ → drop カウンタ + Critical）、`enqueuePublicationIntent`（intentQueue_ → ownerChannel 回収 + Failed）。**機能的には plan の要求を満たすが、統一機構の欠如により将来のポリシー変更が各関数の修正を要する**。要検討（Phase 5 の改善候補）。
> 10. **🟡 P2 — MpscBoundedRing の producer hole が cross-type FIFO を弱める**: `processIntent`（ProcessIntent.cpp:36）の `while (intentQueue_.pop())` は、producer hole（別 producer が slot 予約後未書き込み）に遭遇すると false を返して**ループを途中終了**する。後続 intent（hole の後ろ）は次 CoordinatorLoop tick（1ms）まで遅延。計画書の「cross-type FIFO 順序保証」は厳密には成立しないが、producer hole は予約 producer が必ず書き込むため一時的で、実質遅延は 1ms 未満。**許容範囲だが、`isFullyDrained` の pending count チェック（下記 11）と併せて shutdown 時の完全性に注意**。
> 11. **🟡 P2 — `setPendingIntentCount(0)`（processIntent 末尾）により `isFullyDrained` の pending count チェックが実質無効**: `processIntent`（ProcessIntent.cpp:43）が最後に `setPendingIntentCount(0)` を呼ぶため、processIntent 実行後に submitXxx が push した intent の pending count が 0 にリセットされ、**残留 intent が pending count に反映されない**。`isFullyDrained`（AudioEngine.Threading.cpp / ReleaseResources.cpp で消費）の pending count == 0 チェックは、CoordinatorLoop が processIntent を実行した直後は常に true。shutdown の drain 完全性は CoordinatorLoop が `threadShouldExit()` まで回り続けることで担保されるが、**pending count チェック単独では残留 intent を検出できない**（retireBacklogCount 等の他カウンタは機能する）。要検証: 計画書の INV-6「drain → verify」の完全性を pending count に依存しない方式（queue 空チェック）で担保すべきか、Phase 6 で確認。
> 12. **🟡 P2 — Recovery の coalesce（マージ）が未実装（計画書:860 との乖離）**: 計画書は「Recovery は drop 禁止。ただし coalesce 可能（同一 quarantinedHandle の複数 Recovery は 1 件にマージ）」と定義。検証結果、`submitRecoveryRequest`（ISRRuntimePublicationCoordinator.cpp:643-669）は同一 quarantinedHandle の重複 Recovery を**マージせず**、それぞれ個別に `recoveryIntentQueue_` へ push する。coalesce 機能は存在しない。**ただし**、drop 時の HealthEvent 相当（`recoveryIntentDropCount_` → Critical 昇格）は計画書:860 の「Builder Work Queue 側の overflow は HealthEvent」に整合。`recoveryIntentQueue_`（SPSC, capacity 256）が full になるのは Consumer（Builder Loop）が遅延した場合のみで、実用上 coalesce が必要なケースは稀。**要検討**: 同一 quarantinedHandle の重複 Recovery 検出・マージを Phase 5 で実装するか、accept するか判断（Phase 5 の改善候補）。
> 13. **🔴 P1 — `requestReclaim` の TOCTOU により slot リーク（六次レビューで発見・修正）**: `requestReclaimHandle`（AudioEngine.h）と `drainDeferredRetireQueues`（AudioEngine.Retire.cpp）は epoch 事前チェック（`retireEpoch < minReaderEpoch`）後に `requestReclaim` を呼ぶ。しかし `requestReclaim`（ISRRuntimePublicationCoordinator.cpp:584-610）は**内部でも epoch 再確認**する。事前チェックと内部チェックの間に epoch が進んだ場合（TOCTOU）、`requestReclaim` は `reclaimInFlightCount_` +1 して return し、**handle は保留リストに再登録されず喪失 = slot リーク**。**2026-08-08 六次レビューにて修正**: `requestReclaim` を `bool` 返しに変更（true=reclaim 完了, false=遅延）。`requestReclaimHandle` と `drainDeferredRetireQueues` は false を受け取った場合、handle を `pendingReclaimHandles_` に再登録する（次の drain サイクルで再試行）。これにより TOCTOU による slot リークを構造的に排除。
> 14. **🔴 P1 — 最近の修正（pendingReclaim / Recovery 配線 / TOCTOU 修正）がテストカバレッジ欠落（INV-3/INV-5 のテスト未実装）**: 計画書（第13章:1181）は「各 invariant に対応するテストを `src/tests/invariant_*.cpp` 形式で追加」と要求。しかし検証結果、**`invariant_*.cpp` 形式のテストは存在せず**、以下の最近の修正が**テスト・ハーネスで一切参照されていない**: (a) `requestReclaim` bool 化 + `pendingReclaimHandles_` 再試行（INV-3 retire 順序 / slot リーク防止）、(b) `retireByHandle` への `requestReclaimHandle` 追加（Observe 経路 slot リーク修正）、(c) Recovery 発行経路（`QuarantineIntentHandler` → `submitRecoveryIntent`）、(d) `recoveryIntentDropCount` 監視（INV-5）。既存テスト（`ISRSemanticValidationTests.cpp:609-622` の `testRecoveryRequestEnqueueAndPop`、`MpscBoundedRingTests`）は成功パスのみ検証し、最近の修正の**回帰・エッジケース（TOCTOU、pendingReclaim 再試行、Recovery drop 監視）を検証しない**。**要対応**: Phase 6/7 で (a)-(d) の単体テスト + `invariant_INV-3`/`invariant_INV-5` を追加（計画書第14章の受け入れ条件 :1280-1283, :726-730 に対応）。
> 15. **🟡 P2 — 最近の修正のコンパイル検証（2026-08-09 に実施・成功）**: **検証結果**: `build.bat Release icx nopause` でフルリビルド成功（ConvoPeq.exe 48.5MB / AudioEngineHarness.exe 12.6MB / 全テスト exe 27個生成）。verbose フルビルド（`cmake --build --config Release -- -v -j 1`）で **C4834 以外のワーニングはゼロ**（`-Wall -Wextra` フラグ付き icx で確認）。私が変更した6ファイル（AudioEngine.Retire.cpp / ISRRuntimePublicationCoordinator.cpp / DSPLifetimeManager.cpp / AudioEngine.Commit.cpp / AudioEngine.CtorDtor.cpp / AudioEngine.Init.cpp）は単体コンパイルでワーニング0件を確認。**補足**: (a) ビルドディレクトリ `build-icx` が WSL/Windows FS 競合で破損（`.obj` がアクセス拒否で削除不能）→ `build-icx-old-broken` にリネームし、新しい build-icx で再ビルド成功。(b) フルクリーン後の初回ビルドで `AudioEngine.Processing.AudioBlock.cpp` が一時 FAILED（build-warn3.log:299/407）→ リトライで成功（AudioBlock.cpp は git 上未変更で、recent fixes とは無関係の環境問題）。**結論**: recent fixes のコンパイル検証は完了（エラー0・C4834以外のワーニング0）。
> 16. **🟡 P2 — shutdown 時の `pendingReclaimHandles_` 処理（エッジケース）**: `~AudioEngine`（AudioEngine.CtorDtor.cpp:185-224）の DrainRetire フェーズで `drainDeferredRetireQueues(true)`（:222）が保留 reclaim 再試行を実行する。Audio Thread 停止後は `activeReaderCount()==0`（:201 で待機）→ `minReaderEpoch` が最新 epoch に進むため、通常は `requestReclaim` が成功し `reclaimInFlightCount_` が 0 に戻る。**ただし**、stuck Reader が存在する異常系では `requestReclaim` が unsafe を返し続け `reclaimInFlightCount_` が 0 に戻らず、`markShutdownComplete`（:224）→ `isFullyDrained`（reclaimInFlightCount_==0 要求）が false → `CoordinatorState::Faulted` になる可能性。`detectStuckReaders` が timeout で解消する想定だが、**Phase 6（shutdown 検証）で「pendingReclaimHandles_ が shutdown 完了前に空になること」をテストで固定すべき**（INV-6 関連）。
> 17. **🔴 P1 — `QuarantineIntentHandler` が quarantine 失敗時にも Recovery を発行（配線漏れ・六次レビューで発見・修正）**: `QuarantineIntentHandler::handle`（ProcessIntent.cpp:73-100）は `executeQuarantine` の**戻り値を無視**し、`!request.handle.isNull()` のみで Recovery を発行していた。quarantine に失敗した handle（例: 既に隔離済み・handle 無効）に対して Recovery を発行すると、**quarantined されていない DSP の無意味な世界再構築**を引き起こす。**2026-08-08 六次レビューにて修正**: `executeQuarantine` の戻り値 `QuarantineResult.stateChanged`（ISRRuntimePublicationCoordinator.h:48）を確認し、**`stateChanged == true` かつ handle 非 null の場合のみ** Recovery を発行（ProcessIntent.cpp:82-99）。`stateChanged` は「handle 有効かつ quarantine() 成功」を意味する（executeQuarantine:619-622 実測）。`auditLogged` は State 変更を妨げない（計画書:631 — audit 失敗は State 確定後）。HANDLER-1（判定は QuarantineService が唯一行う）に整合。
> 18. **🟡 P3 — Recovery world モデルと計画書の「spec 除外」設計の乖離（単一 DSP world では暗黙成立）**: 計画書（:691-703）は Recovery build を「現在の全 spec 集合から quarantined 除外（`recoverySpec.erase(qHandle)`）した世界の再構築」と設計。しかし実装（RebuildDispatch.cpp:911-972）は `runtimeBuilder.build()`（RuntimeBuilder.cpp:443 — **単一 DSPCore 生成**）で 1 つの DSP を build し、`enqueuePublicationIntentForRuntimeCommit` で publish する。**ConvoPeq の RuntimeWorld = `RuntimeState` = 単一 DSPCore 構成**（AudioEngine.h:329 `using RuntimePublishWorld = RuntimeState`, :443 単一 DSPCore）のため、`recoverySpec.erase(qHandle)` は単一 DSP world では**暗黙的に成立**する（quarantined DSP は active/fading から除去済み → 新 world に含まれない。RebuildDispatch.cpp:961-962 のコメント主張どおり）。**`recoverySpec.erase(qHandle)` の設計は将来の複数 DSP world 拡張用であり、現行実装に配線漏れはない**。**要検証**: 将来 world が複数 DSP 構成に拡張された場合、Recovery build の spec 集合管理を再設計すること（Phase 5 の将来拡張メモ）。
> 19. **🟢 P3 — `currentBuildSnapshot_` の初期状態（sealed=false）の挙動確認**: `currentBuildSnapshot_`（AudioEngine.h:4614）は初期値 `RuntimeBuildSnapshot{}`（sealed=false）。初回 publish 前に quarantine が発生した場合、`getCurrentBuildSnapshotForRecovery()` は sealed=false の snapshot を返し、Builder Loop の `recovery->buildSource.sealed` チェック（RebuildDispatch.cpp:917）で**Recovery が skip される**。これは正しい動作（build できる構成がない）。**軽微な無駄**: `submitRecoveryIntent` が `recoveryPending=true` + `rebuildCV.notify_all()` を実行し、無意味な起床が発生するが、機能上の問題はない。**要対応なし**（Phase 5 の最適化候補）。
> 20. **🟢 P3 — graphify 解析 + trivially_copyable 静的検証（検証結果の記録）**: (a) **graphify**: `RecoveryIntentHandler`（ISRIntentDispatcher.h:43）は `IntentHandler` を継承し `.handle()` メソッドを持つ（graph.json は静的抽出のため、ProcessIntent.cpp:122 の実装本体への依存は抽出されない — 動的配線は六次レビューで別途確認済み）。(b) **trivially_copyable**: `BuildInput` の全メンバは組み込み型のみ（POD、NSDMI は copy constructor を trivial に保つ）→ `RuntimeBuildSnapshot` → `RecoveryPayload` → `Intent` は全て trivially copyable。`static_assert(std::is_trivially_copyable_v<convo::RuntimeBuildSnapshot>)`（ISRRuntimePublicationCoordinator.h:184）と `static_assert(std::is_trivially_copyable_v<Intent>)`（:258）は成立見込み。`Intent` の union が NSDMI 付き `RecoveryPayload` を持つためデフォルトコンストラクタ削除 → 明示的デフォルトコンストラクタ（:239-242）で対応済み。**ただし**: 実コンパイル確認はビルド未検証（項目15）のため、`build.bat Release` で確認すること（clang では `ISRDSPHandle.h:209` の `static_assert(atomic<DSPHandle>::is_always_lock_free)` が MSVC 専用分岐で失敗 — 既存の問題で、変更箇所ではない）。(c) **MpscBoundedRing producer hole と pendingIntentCount の相互作用**: `pop`（MpscBoundedRing.h:98-112）は `diff != 0` で false（producer hole は push() 実行中のみ一時的に存在、push() が true を返す前に解消）。`submitXxx` は `push()` 成功後に `pendingIntentCount_ + 1` するため、producer hole で intent は失われず、次 tick の pop で取得される。`setPendingIntentCount(0)` との相互作用による `isFullyDrained` 誤判定は、CoordinatorLoop が次 tick で pop 再試行するため実害なし（項目11のエッジケースを参照）。

> **残余リスク（文書化・対応推奨 — 六次レビューで実コード検証済み）:**
>
> | # | 残余リスク | 検証結果 | 対応推奨 |
> |---|-----------|---------|----------|
> | R1 | **`recoveryIntentQueue_` は SPSC**（`LockFreeRingBuffer<RecoveryIntent, kRecoveryIntentQueueCapacity>`、ISRRuntimePublicationCoordinator.h:434）。Producer=CoordinatorLoop（submitRecoveryRequest 経由）、Consumer=Builder Loop。**将来 quarantine→recovery を直接（Timer 等から）呼ぶ場合は MPSC 化が必要** | ✅ 妥当。Producer は現在 CoordinatorLoop のみ（QuarantineIntentHandler → submitRecoveryIntent → submitRecoveryRequest が CoordinatorLoop スレッドで実行）。MPSC 化は将来拡張時のみ必要 | Phase 5 の将来拡張メモ |
> | R2 | **`observeIntentQueue_`/`observeFallbackQueue_` は Observe 統合後デッドメンバ（除去推奨・無害）** | ✅ 妥当だが **除去は済んでいる**。実コード（ISRRuntimePublicationCoordinator.h）に宣言は存在せず、コメントのみ（:418, cpp:541, ProcessIntent.cpp:15,23,24）。六次レビューで**削除済み**（項目3参照）。コメントは残存（将来の歴史的記録として無害） | 対応済み。コメントは残置（無害） |
| R3 | **Recovery 経路はプロダクション producer 未接続のため、第6・7件の修正は将来配線時に有効化（潜在バグの事前排除）** | 🔴 **不正確（六次レビューで修正済み）**。`QuarantineIntentHandler::handle`（ProcessIntent.cpp:73-100）が `executeQuarantine` 成功後（`qResult.stateChanged`）に `submitRecoveryIntent` → `submitRecoveryRequest` を呼ぶ**プロダクション経路が接続済み**。さらに `RecoveryIntentHandler::handle`（:117-123）も将来の intentQueue_ 経由拡張用に実装済み。**第6件（Recovery 発行配線）と第7件（Recovery push drop 修正）は有効化済み** | 対応済み。記述の更新が必要 |
| R4 | **retire 順序逆転（`retireDSPHandleForRuntime` の shutdownReclaim が enqueue より先）は残るが、quarantine fallback によりリーク/UAF は構造的に排除済み**（reclaim は slot の状態遷移のみで DSPCore を解放しないため UAF なし・失敗時は退避ストアが確実に回収）。**完全対応（requestReclaim 一本化）は大規模リファクタのため保留** | ✅ 妥当（部分修正済み）。runtime 経路（`retireDSPHandleForRuntime`）から `shutdownReclaim` は**排除済み**（AudioEngine.h:4199 は requestReclaimHandle 呼び出しに変更）。`shutdownReclaim` は shutdown 専用（AudioEngine.h:2027 CacheMap::dtor, shutdownPhase>=Destroy 時のみ、ISRDSPHandle.h:171 定義のみ）。retire→enqueue の順序は、`requestReclaimHandle`（epoch 安全確認 → requestReclaim → reclaim）と `enqueueWithRetry`（DSPCore* 削除予約）に分離され、順序契約が保たれる。UAF/リークは quarantine fallback + RetireQuarantineStore で構造的に排除 | 対応済み（runtime 経路）。`shutdownReclaim` は shutdown 専用として正当 |
| R5 | **bootstrap `ignoreUnused(result)`（Init.cpp:55）: bootstrap publishWorld 失敗時に無視。失敗時の null world クラッシュ可能性は既存の軽微なロバストネス事項（bootstrap は稀にしか失敗せず、次の操作が検出する）。今回の変更とは無関係** | ✅ 妥当。Init.cpp:55 に `juce::ignoreUnused(result)` を確認。bootstrap publishWorld 失敗時の null world は、直後の `submitRebuildIntent` が検出する想定（Worker が null world を見て rebuild）。軽微なロバストネス事項 | 要対応なし（既知）。必要なら `jassert` 追加（Phase 9） |
| R6 | **BlockDouble の `finalizeCrossfadeMixPath(..., false)`: double パスで dryScaleGain_ が未リセット（AudioBlock は true）。事前存在の差異・work88 対象外。将来の確認事項として文書化** | ✅ 妥当。BlockDouble.cpp:434 は `finalizeCrossfadeMixPath(dsp, fading, false)`、AudioBlock.cpp:458 は `true`。double パスで `resetDryScaleGain=false` のため dryScaleGain_ がリセットされない。これは事前存在の差異で、BUG-028 の変更（complete() で dryScaleTarget_ を atomic publish）とは独立。ただし、BUG-028 修正（RT 側 finalizeCrossfadeMixPath が dryScaleGain_ を直接書く）との整合を**将来確認すべき**（double パスが BUG-028 の RT-only ownership 契約に適合するか） | Phase 6（soak）で確認事項として文書化 |
>
> **残余リスクの検証方法（六次レビュー）**: 上記 R1-R6 は実コード照合で検証した。R2 は削除済み、R3 は修正済み（プロダクション経路接続済み）、R4 は runtime 経路の `shutdownReclaim` 排除済み（shutdown 専用に限定）。R1/R5/R6 は将来拡張・既知事項として妥当。
