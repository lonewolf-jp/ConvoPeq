...
> **⚠️ 本書 (v1) は実施済みではない。現行ソース準拠版は `null_test_procedure_v2.md`（v2.1, 2026-09-11）を参照すること。**
> v1 の層構成前提（IR=2000 → L0 のみ、IR=5000 → L0=2048+L1=2952）は現行ソースと一致して正しいが、`align = 64`（§2.2）は B13 遅延補償実装（2026-07 以降）後の現行実装に対して無効であり、判定基準も v2 で再設計された。
