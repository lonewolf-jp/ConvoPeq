...
バグレポートの全7項目について、**全件が実コードと一致する正確な指摘であることを確認した**。各バグの重大度評価、根拠となるコード引用、修正パッチの方向性はいずれも妥当である。以下に、詳細検証で判明した補足・訂正事項を記す。

---

## Bug 1 [Critical] `Layer::delayLineBuf` リーク — ✅ **確認・修正推奨**

| 検証項目 | 結果 |
|---------|------|
| `LayerAllocSizes` に `delayLineBuf` フィールドがない | ✅ `MKLNonUniformConvolver.h:71-87` で確認 |
| `freeAll()` 診断ビルド分岐に `freeTracked(delayLineBuf,...)` がない | ✅ `MKLNonUniformConvolver.cpp:325-344` で確認 |
| 非診断分岐には `mkl_free(delayLineBuf)` が存在 | ✅ `MKLNonUniformConvolver.cpp:361` で確認 |
| `SetImpulse()` で生 `mkl_malloc` 使用（`DIAG_MKL_MALLOC` 未使用） | ✅ `MKLNonUniformConvolver.cpp:1108` で確認 |
| `allocSizes.delayLineBuf` が未設定 | ✅ 確認 |

**補足**: `freeTracked()` は `DiagnosticsConfig.h` で定義され、最初に `if (p)` のポインタnullチェックを行う。`allocSizes` の値が0でもnullptrチェックで保護されるため解放漏れにはならないが、**診断統計（`NucDiagnosticsSnapshot.totalBytes()`）に delayLineBuf のサイズが一切カウントされない**という二次的影響がある。修正パッチの3箇所（ヘッダのフィールド追加・確保側の`DIAG_MKL_MALLOC`統一・解放側の`freeTracked`追加）はすべて妥当。

---

## Bug 2 [Critical] Retireキュー枯渇によるポインタロスト — ✅ **確認**

| 検証項目 | 結果 |
|---------|------|
| ① `DSPLifetimeManager::retire()` でリトライ失敗後 return | ✅ `DSPLifetimeManager.h:54-67` で確認 |
| ② `AudioEngine::retireDSP()` → `enqueueDeferredDeleteNonRtWithResult()` でリエンキューなし | ✅ `AudioEngine.h:3916-3948` で確認 |
| ③ `RuntimePublicationCoordinator::enqueueRetire()` でリトライなし | ✅ `ISRRuntimePublicationCoordinator.cpp:132-152` で確認 |
| ④ `RefCountedDeferred::release()` でRT時リトライ禁止 | ✅ `RefCountedDeferred.h:32-53` で確認 |

### ★ 追加調査で判明した重要な補足事項

**1. `ISRRetireRouter::enqueueRetire()` に内部リトライ機構が存在する**
```cpp
// src/audioengine/ISRRetireRouter.cpp (L110-120)
// ★ Practical-4: QueueFull → 同期的 tryReclaim を１度だけ試行（レート制限付き）
// 500ms クールダウン付きで tryReclaim → 再試行
```
これにより、呼び出し元のリトライに加えてルーター層でもう1回リトライが行われる。つまり実効的には**最大2回の tryReclaim** が試行される。ただし、両方失敗した場合はやはりポインタはロストする。

**2. `DeferredRetireFallbackQueue`（ミューテックス保護セカンダリキュー）が存在する**
```cpp
// src/core/DeferredRetireFallbackQueue.h
// SoftLimit: 1000, HardLimit: 2000, 最大3回リトライ, 50MBメモリ制限
```
このフォールバックキューは `ISRRetireRuntime`（低レベルRT退避システム）が使用するもので、**`DeferredDeletionQueue`（Bug 2 の対象キュー）とは異なる系統**である。つまりBug 2の分析対象である `DSPLifetimeManager` 等の経路は `DeferredDeletionQueue` のみを使用し、`DeferredRetireFallbackQueue` の保護を受けない。

**3. 実運用上のリスク評価**
- `DeferredDeletionQueue` の容量は 4096 エントリ
- 各エントリは `DSPCore*`（数十〜数百バイト）または `RefCountedDeferred` オブジェクト
- キューが満杯になる条件: 4096個以上のオブジェクトが同時に retire され、かつ `tryReclaim` でいずれも解放できない（全 Reader が stuck）
- この状況は「Reader Thread が長時間処理中で epoch を進めない」場合に起こり得る
- `EpochDomain::detectStuckReaders()` は 10秒/30秒 の滞留閾値を持ち、Quarantine 機構で stuck Reader を隔離可能
- **総合すると、正常運用下での発生確率は低いが、系統的リークの原因となり得る設計上のリスクは確かに存在する**

**4. 実測推奨**: `overflowCount()` は `AudioEngine.Timer.cpp:925` で監視対象として読み出されている。この値が非ゼロであれば確実にポインタロストが発生している。まずこの値を確認することを推奨。

---

## Bug 3 [High] EQ `fastTanh` しきい値不整合 — ✅ **確認・修正推奨**

| 検証項目 | 結果 |
|---------|------|
| `kClipThreshold = 4.5` で係数は `27.0/9.0`（3.0用）のまま | ✅ `EQProcessor.Processing.cpp:86-112` で確認 |
| スカラー版は `>=4.5` で早期 return 1.0 | ✅ 確認 |
| SIMD版はクランプ後に同じ式を評価（早期returnなし） | ✅ 確認 |
| 呼び出し元で経路が分岐する（processBand vs processBandStereo） | ✅ `EQProcessor.Processing.cpp:153-159, 237-247` で確認 |

### ★ 修正方針の補足

**TanhApprox の検証**: `DSPCoreDouble.cpp` の `TanhApprox` 名前空間に高次Padé近似が存在する。
```
NUM_A = 10395.0, NUM_B = 1260.0, NUM_C = 21.0
DEN_A = 10395.0, DEN_B = 4725.0, DEN_C = 210.0
CLIP_THRESHOLD = 4.5
```
これは **5次/6次 Padé近似**（報告書の「7次/7次」は誤り。正しくは 5次/6次）で、`x=4.5` で約 0.99927 に収束する。EQの `fastTanhScalarOutput`（3次/2次 Padé）をこの `TanhApprox::fastTanh(double)` に差し替えることで、係数としきい値の整合が取れた高品質サチュレーションが実現できる。

**SIMD版**: `DSPCoreDouble.cpp` 内に `fastTanhV256(__m256d)` が存在するかについては未確認だが、`TanhApprox` の係数を使用した `__m256d` 版の実装は `DSPCoreDouble.cpp:184-190` に存在するAVX2 Tanhブロックを流用できる。

したがって、修正パッチの方向性として以下を推奨する（報告書には2つの選択肢が提示されている）:
1. **最小修正**: 閾値を 3.0 に戻す（係数と整合。ただし Low Shelf +12dB で過剰クリップ）
2. **本修正**: `TanhApprox`（5次/6次, 4.5対応）に差し替える（推奨）

---

## Bug 4 [High] `ProgressiveUpgradeThread` FTZ/DAZ 未設定 — ✅ **確認・修正推奨**

| 検証項目 | 結果 |
|---------|------|
| `ProgressiveUpgradeThread::run()` に FTZ/DAZ 設定なし | ✅ `ProgressiveUpgradeThread.cpp:73-92` で確認 |
| `IRConverter::convertFile()` に FTZ/DAZ 設定なし | ✅ `IRConverter.cpp:236-257` で確認 |
| `IRDSP::resampleIR()` の内部実装 | ✅ `IRDSP.cpp` で確認（r8brain使用、MKL VML不使用） |
| 他の全バックグラウンドスレッドで FTZ/DAZ 設定あり | ✅ 報告書のgrep結果の通り |

### ★ 追加調査結果

**`IRDSP::resampleIR()` の詳細**:
- r8brain `CDSPResampler` を使用したFIRリサンプリング
- チャンク処理（chunkSize: 1024〜8192）で並列処理（`std::async`）
- IR（インパルスレスポンス）は指数的減衰 → **デノーマル典型発生源**
- MKL VML関数（`vdExp` 等）は直接使用していない

**影響評価**: 本バグの実害は「ウォームアップ窓の延長」に直結する。ユーザーのメモリ調査メモに記載の「60〜70秒ウォームアップ窓での2エンジン共存」は、このスレッドの実行時間に比例する。FTZ/DAZ未設定によるデノーマルペナルティ（数十〜数百倍のレイテンシ）が加わると、ウォームアップ時間がさらに伸び、**2エンジン共存時間が延びてメモリ削減効果を相殺する**。修正優先度は「High」の評価が妥当。

---

## Bug 5 [Medium] `reclaim()` 先読みスキャン到達不能 — ✅ **確認・修正推奨**

| 検証項目 | 結果 |
|---------|------|
| `scanPos` が常に `deqPos` と等しい状態でのみ `else` に到達 | ✅ `DeferredDeletionQueue.h:107-167` で制御フロー完全追跡済み |
| `else` 節内 `if (!canDelete) break;` が常に発動 | ✅ 確認 |
| `kMaxScan` 先読みロジックがデッドコード | ✅ 確認 |
| MPMC enqueue による epoch 逆転可能性 | ✅ 分析済み（後述） |

### ★ epoch 逆転の実運用可能性

**`DeferredDeletionQueue` は MPMC**（複数 Producer が同時に `enqueue()` 可能）。各 Producer は独立に `currentEpoch()` を読んで epoch 値を取得する。スレッドスケジューリングの都合で、「epoch=6 を読んだスレッド A」が先に enqueue し、「epoch=5 を読んだスレッド B」が後から enqueue する可能性は否定できない。これが実際に起きた場合、キュー内で epoch 順序が逆転する。

**影響**: 先頭に「epoch=7（reclaim不可）」が居座ると、その後ろの「epoch=5（reclaim可能）」が `kMaxScan=1024` の先読み予算があっても回収されない（先読みロジックがデッドコードのため）。これにより reclaim の遅延 → キューの見かけ上の飽和 → **Bug 2 の QueuePressure/QueueFull を誘発しやすくなる**という連鎖が起こり得る。

**修正方針**: 報告書の2つの選択肢（「epoch逆転が起こらないならデッドコード削除」「起こり得るなら先読みロジックを実装し直す」）はいずれも妥当。ただし、epoch逆転の発生有無を実測で確認するには `overflowCount()` のモニタリングが有効。

---

## Bug 6 [Low/Medium] `delayLineReadAdd()` の `std::abs` — ✅ **確認・修正推奨**

| 検証項目 | 結果 |
|---------|------|
| 同一ファイル内に `absNoLibm` が定義されている | ✅ `MKLNonUniformConvolver.cpp:40-43` で確認 |
| `addScaledFallback`（同一 `Get()` 内）で `absNoLibm` を使用 | ✅ `MKLNonUniformConvolver.cpp:1655` で確認 |
| `delayLineReadAdd()` で `std::abs` を使用 | ✅ `MKLNonUniformConvolver.cpp:1756, 1767` で確認 |
| 関数呼び出し階層: `Get()` → `delayLineReadAdd()`（同一Audio Threadパス） | ✅ 確認 |

報告書のパッチ（`std::abs` → `absNoLibm`）は妥当。MSVCのデフォルト設定では `std::abs(double)` はインライン化されるが、`/fp:strict` 等の設定でlibm呼び出しに変わるリスクを排除する意味がある。

---

## Bug 7 [Low] `SetImpulse()` ブレース漏れ — ✅ **確認・実害なし確認済み**

| 検証項目 | 結果 |
|---------|------|
| `if (!l.isImmediate)` にブレースなし | ✅ `MKLNonUniformConvolver.cpp:972-976` で確認 |
| 診断ブロックが `isImmediate` ガードの外にある | ✅ 確認 |
| L0（immediate）で `allocSizes.tailOutputBuf` に誤値が入る | ✅ 確認 |
| 実害がないことを `freeTracked()` のnullptrチェックで確認 | ✅ `DiagnosticsConfig.h` で確認 |

報告書の修正パッチはコードの意図を明確にする改善として妥当。

---

## 誤検知除外項目の確認結果

報告書で「誤検知として除外」とされた5項目について、実際のソースコードを確認した:

| # | 項目 | 報告書の主張 | 検証結果 |
|---|------|------------|---------|
| 1 | AoS/SoA二重保持 | 既に修正済み | ✅ `MKLNonUniformConvolver.h` で確認。`irFreqDomain`/`fdlBuf` はスクラッチ用小サイズ |
| 2 | `processOutput()` NaN/Inf | Float経路も共有ヘルパー呼び出しで保護 | ✅ `DSPCoreIO.cpp:338-400` でAVX2 NaN/Infスクラブ確認 |
| 3 | `pushAdaptiveCaptureBlocks` 重複 | 匿名名前空間で内部リンケージのため問題なし | ✅ `DSPCoreDouble.cpp` と `DSPCoreIO.cpp` に別定義確認。翻訳単位ごとの独立シンボル |
| 4 | `softClipBlockAVX2` prevSample | ADAA用に意図的保持 | ✅ Float版コメントで「ADAA用にフィールド残す」確認 |
| 5 | Double版とFloat版の実装差異 | 両者とも最終NaN/Infスクラブを通過 | ✅ Float: `processOutput()` → `DSPCoreIO.cpp:338`。Double: `processDoubleToBuffer()` に自前スクラブ |

**結論**: 5項目すべて、報告書の「問題なし」判断は正確である。

---

## 未精査ファイルに関する補足

報告書の「未精査」リストに含まれるファイル群のうち、特に以下のものはBug 2（Retireキュー枯渇）との関連で優先度が高い:

- **`SafeStateSwapper.h`**: 状態切り替え時に retire を伴う可能性
- **`ConvolverProcessor.*.cpp`（LoaderThread, Lifecycle, MixedPhase 等）**: NUCのラッパーとしてretire・メモリ管理に直接関与
- **`SnapshotCoordinator.h` / `SnapshotFactory.cpp`**: 診断スナップショット生成時に Bug 1 の統計漏れの影響を受ける

---

## 総合推奨事項

1. **最優先（Critical）**: Bug 1 + Bug 2 の修正。診断ビルドのメモリリークと系統的リークパターンは実運用に直結する。
2. **優先（High）**: Bug 3 + Bug 4 の修正。EQ品質とウォームアップ時間に影響。
3. **中優先（Medium）**: Bug 5 + Bug 6 の修正。デッドコード削除とコーディング規約統一。
4. **低優先（Low）**: Bug 7 の修正。コードの意図明確化。
5. **実測推奨**: `overflowCount()` の値を確認し、非ゼロであれば Bug 2 が実際に発生している証拠となる。
6. **継続調査候補**: `NoiseShaperLearner.cpp`（CMA-ES実装1300行超）、`ConvolverProcessor.*` 一式。
...
今回精査したのはリアルタイム/メモリに直結する以下のファイル群です:
`MKLNonUniformConvolver.cpp/h`, `AudioEngine.Processing.DSPCoreDouble/Float/IO/BlockDouble/AudioBlock/DSPCoreLifecycle.cpp`, `EQProcessor.Processing.cpp`, `CustomInputOversampler.cpp`, `DspNumericPolicy.h`, `Fixed15TapNoiseShaper.h`, `DiagnosticsConfig.h`, `core/EpochDomain.h`, `DeferredDeletionQueue.h`, `DeferredFreeThread.h`, `ISRRetireRouter.cpp/h`, `DSPLifetimeManager.h`, `AudioEngine.h`（retire関連）, `ISRRuntimePublicationCoordinator.cpp`, `RefCountedDeferred.h`, `ProgressiveUpgradeThread.cpp`, `IRConverter.cpp`, `CacheManager.cpp`。

以下は261ファイル中、**未精査**です: `AllpassDesigner.cpp/h`, `OutputFilter.cpp/h`, `TruePeakDetector.cpp`, `LoudnessMeter.cpp`, `PsychoacousticDither.h`, `LatticeNoiseShaper.h`, `FixedNoiseShaper.h`, `NoiseShaperLearner.cpp`（CMA-ES本体, 1300行超）, `ConvolverProcessor.*.cpp`（十数ファイル）, `SafeStateSwapper.h`, `SnapshotFactory.cpp`, `SnapshotCoordinator.h`, UI関連一式（`MainWindow.cpp`, `DeviceSettings.cpp`, `ConvolverControlPanel.cpp` 等）。これらについても同水準で継続調査可能です。「つづけて」とご指示いただければ、上記の未精査ファイル群（特に `NoiseShaperLearner.cpp` のCMA-ES実装と `ConvolverProcessor.*` 一式）から着手します。
