...
### 13.1 CI fail-stop 検査の必須化

以下の禁則は、静的検査を CI fail-stop で実施することを mandatory とする。

- Rule-2（RT path 禁則）
- Rule-4（worker の runtime pointer 参照禁止）
- Rule-11（publication resurrection 禁止）
- Rule-23（shutdown drained 後の再起動禁止）

### 13.2 `shutdownRuntime_` 役割の明確化

Rule-8 の統合先である `shutdownRuntime_` は、shutdown/drain 期間の runtime 可視性を管理する参照点として扱う。
独立 state machine の追加は禁止を維持する。

### 13.3 Rule-12 追補（saturation 観測必須）

saturation state 遷移の観測として、以下カウンタを mandatory とする。

- `saturationEnterCount`
- `saturationExitCount`

### 13.4 Rule-19 追補（execution-local 判定基準）

execution-local state は次をすべて満たすものに限定する。

1. lifetime が RT 実行局所（または明示的に非共有）である
2. 他スレッドから直接参照されない
3. publication 経路に乗らない

上記を満たさない mutable state は shared mutable authority とみなし、Rule-19 違反とする。
