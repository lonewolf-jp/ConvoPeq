...
- `2026-05-20 | Stage F batch-98 | files: src/audioengine/AudioEngine.h,src/core/RuntimePublicationCoordinator.h | changed: RuntimePublicationBridge::buildWorldAndPublishTransition を buildRuntimePublishWorld へリネームし、RuntimePublicationCoordinator を Bridge&& + publishConstructedWorld helper で一様 publish 経路に整理 | verify: errors=No scan=OK build=OK(Release) | risk: 低（命名整合 + 重複 publish 手順の集約） | next: Stage F batch-99（DSP helper の dead order 引数除去）`
- `2026-05-20 | Stage F batch-99 | files: src/audioengine/AudioEngine.h | removed-legacy: loadCurrentDSP/exchangeCurrentDSP/publishCurrentDSP/loadFadingOutDSP/exchangeFadingOutDSP/publishFadingOutDSP の std::memory_order order 既定引数と juce::ignoreUnused(order) を除去 | verify: errors=No scan=OK build=OK(Release) | risk: 低（未使用引数除去のみ、呼び出し実引数互換確認済み） | next: Stage F batch-100（RuntimePublicationCoordinator 周辺の thin wrapper 追加削減候補を精査）`
- `2026-05-20 | Stage F batch-100 | files: src/audioengine/AudioEngine.h | removed-legacy: 未参照となっていた buildWorldAndPublishTransition 1:1 wrapper を削除し、buildRuntimePublishWorld へ統一 | verify: errors=No scan=OK build=OK(Release) | risk: 低（dead wrapper 削除のみ、呼び出し参照ゼロ確認済み） | next: Stage F batch-101（AudioEngine.h 周辺の薄い重複 helper 継続監査）`
- `2026-05-20 | Stage F batch-101 | files: src/audioengine/AudioEngine.h | changed: adoptAndBuildPublishWorld（engine helper）を削除し、RuntimePublicationBridge::adoptAndBuildPublishWorld で publishCurrentDSP + buildRuntimePublishWorld を直列化 | verify: errors=No scan=OK build=OK(Release) | risk: 低（1:1転送 helper 削除のみ、順序は従来同等） | next: Stage F batch-102（冗長 helper の追加削減）`
- `2026-05-20 | Stage F batch-102 | files: src/audioengine/AudioEngine.h | removed-legacy: publishCurrentDSPAndTakeOwnership を削除し、Bridge 側は publishCurrentDSP を直接呼び出しへ簡素化（重複代入を解消） | verify: errors=No scan=OK build=OK(Release) | risk: 低（実処理は publishCurrentDSP 単体と等価） | next: Stage F batch-103（bridge/coordinator 境界の API 薄化）`
- `2026-05-20 | Stage F batch-103 | files: src/audioengine/AudioEngine.h,src/core/RuntimePublicationCoordinator.h | changed: bridge の retire/reset 二段APIを retireRuntimePublishWorldNonRt(world, resetRevision) に統合し、clear/publish 側から true/false 指定で利用 | verify: errors=No scan=OK build=OK(Release) | risk: 低（clear時のみ revision reset の既存意味を保持） | next: Stage F batch-104（coordinator 内の一時変数削減）`
- `2026-05-20 | Stage F batch-104 | files: src/core/RuntimePublicationCoordinator.h | changed: adoptAndPublish の worldOwner 一時変数を削除し、publishConstructedWorld(bridge_.adoptAndBuildPublishWorld(...)) へ直結 | verify: errors=No scan=OK build=OK(Release) | risk: 低（所有権移動の経路簡素化のみ） | next: Stage F batch-105（AudioEngine.h セクション整形と残存 thin wrapper 継続監査）`
- `2026-05-20 | Stage F batch-105 | files: src/audioengine/AudioEngine.h,src/core/RuntimePublicationCoordinator.h | changed: adoptAndBuildPublishWorld を削除し、RuntimePublicationCoordinator::adoptAndPublish で publishCurrentDSP + buildRuntimePublishWorld を直列化して helper 1本を薄化 | verify: errors=No scan=OK build=OK(Release) | risk: 低（処理順は維持しつつ 1:1 helper を削減） | next: Stage F batch-106（bridge/coordinator の薄い残存 API を継続監査）`
- `2026-05-20 | Stage F batch-101 | files: src/audioengine/AudioEngine.h | changed: adoptAndBuildPublishWorld（engine helper）を削除し、RuntimePublicationBridge::adoptAndBuildPublishWorld で publishCurrentDSP + buildRuntimePublishWorld を直列化 | verify: errors=No scan=OK build=OK(Release) | risk: 低（1:1転送 helper 削除のみ、順序は従来同等） | next: Stage F batch-102（冗長 helper の追加削減）`
- `2026-05-20 | Stage F batch-102 | files: src/audioengine/AudioEngine.h | removed-legacy: publishCurrentDSPAndTakeOwnership を削除し、Bridge 側は publishCurrentDSP を直接呼び出しへ簡素化（重複代入を解消） | verify: errors=No scan=OK build=OK(Release) | risk: 低（実処理は publishCurrentDSP 単体と等価） | next: Stage F batch-103（bridge/coordinator 境界の薄い API 継続監査）`
