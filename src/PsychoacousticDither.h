//============================================================================
#pragma once
// PsychoacousticDither.h ── v0.3 (JUCE 8.0.12 対応)
//
// Ultra Mastering Dither Engine
// 64bit Double 専用 Psychoacoustic Dither RNG
// 構成：
//
//   1. Xoshiro256** (L/R 独立 jump)
//   2. True TPDF Dither
//   3. 12 次 Noise Shaper (Error Feedback Topology)
//      ├ 出力サンプルレート帯域：5 バンド
...
public:
    ...
    // -------------------------------------------------------------------------
    // 12 次 Noise Shaper 係数テーブル
    //
    // インデックス：[SR 帯域 (0-5)][ビット深度プリセット (0=16bit/1=24bit/2=32bit)][タップ (0-11)]
    //
    // SR 帯域：
    //   0: 44.1kHz
    //   1: 48kHz
    ...
    // 係数テーブル
    //
    // 【エラーフィードバック型 NSF の安定性について】
    //   H(z) = c0*z⁻¹ + ... + c11*z⁻¹² は FIR フィルタ (極は全て z=0)。
    //   量子化器により e[n] = tmp[n] - Q(tmp[n]) は常に有界 (|e[n]| ≤ scale/2)。
    //   shapedError[n] = Σ c_k * e[n-k-1] も有界 → 常に BIBO 安定。
    //   「NTF 零点が単位円外 → 不安定」は直接形 (IIR 予測型) NSF の理論であり、
    //   エラーフィードバック型には適用されない。
    //
    ...
    void prepare(double sampleRate, int bitDepth = DEFAULT_BIT_DEPTH) noexcept
    {
        ...
        // N ビット符号付き PCM の量子化ステップは 2 / 2^N = 1 / 2^(N-1)
        ...
        // 係数を実行時バッファにコピー (Audio Thread からはこちらを参照する)
    ...
    // ステレオ時は L/R を並列処理し、CPU パイプライン効率 (ILP) を向上させる。
    // 量子化ステップでは SSE4.1 を使用して L/R を同時に丸める。
    inline void processStereoBlock(double* dataL, double* dataR, int numSamples, double headroom) noexcept
    {
        ...
        if (dataR != nullptr)
        {
            ...
            {
                // 12 次 Shaped Error (コンパイラが FMA に最適化)
                ...
                // Error and State Update (12 タップ シフトレジスタ)
...
private:
    ...
    {
        const double absX = (x < 0.0) ? -x : x;
        return (absX < convo::numeric_policy::kDenormThresholdAudioState) ? 0.0 : x;
    }

    VSLStream rng[MAX_CHANNELS];

    double* shaperStateBuffer = nullptr;
    double scale    = 1.0 / 8388608.0;  // 2^23（24 bit signed PCM デフォルト）
