...
public:
    ...
    //
    // インデックス: [SR帯域(0-5)][ビット深度プリセット(0=16bit/1=24bit/2=32bit)][タップ(0-8)]
    //
    // SR帯域:
    //   0: 44.1kHz
    //   1: 48kHz
    ...
    static constexpr double kCoeffTable[SR_BANDS][3][NS_ORDER] = {
        // ── SR帯域 0: 44.1kHz ──────────────────────────────────────────────────
        // L1ノルム: 42.30 / 35.97 / 29.46
        {
            { 2.93, -5.06,  6.97, -7.66,  7.11, -5.63,  3.96, -2.18,  0.80 }, // 16bit 強め
            { 2.49, -4.30,  5.92, -6.51,  6.05, -4.79,  3.37, -1.86,  0.68 }, // 24bit 標準
            { 2.04, -3.52,  4.85, -5.34,  4.95, -3.92,  2.76, -1.52,  0.56 }, // 32bit 控えめ
        },
        // ── SR帯域 1: 48kHz ────────────────────────────────────────────────────
        // L1ノルム: 41.15 / 34.92 / 28.60
        {
