...
foreach ($entry in $json) {
    ...
    # -external:I<path> → -isystem <path> (CMake generates -isystem<drive:\path> without
    # space on MSVC, which clangd cannot parse; add space for Clang compatibility)
    $cmd = $cmd -replace '-external:I', '-isystem '
    # -isystem<path> (no space, as generated by CMake for MSVC) → -isystem <path>
    $cmd = $cmd -replace '(-isystem)(["\w\\])', '-isystem $2'
    # -external:W0 → remove
    $cmd = $cmd -replace '-external:W0', ''
    # -Qstd:c++20 / -std:c++20 → -std=c++20 (Clang-style)
    $cmd = $cmd -replace '-Qstd:c\+\+20', '-std=c++20'
    $cmd = $cmd -replace '-std:c\+\+20', '-std=c++20'
    # -Qstd:c++17 / -std:c++17 → -std=c++17
