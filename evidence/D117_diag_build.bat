...
cmake -S . -B build -G "Ninja Multi-Config" -DCMAKE_C_COMPILER=cl -DCMAKE_CXX_COMPILER=cl -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCONVOPEQ_ENABLE_RUNTIME_DIAGNOSTICS=ON
if errorlevel 1 (
    echo [ERROR] CMake configure failed
    exit /b 1
)
rem Cache deletion on compiler change wipes -D options: re-run configure to re-apply
cmake -S . -B build -G "Ninja Multi-Config" -DCMAKE_C_COMPILER=cl -DCMAKE_CXX_COMPILER=cl -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCONVOPEQ_ENABLE_RUNTIME_DIAGNOSTICS=ON
if errorlevel 1 (
    echo [ERROR] CMake reconfigure failed
    exit /b 1
)
findstr /C:"CONVOPEQ_ENABLE_RUNTIME_DIAGNOSTICS:BOOL=ON" build\CMakeCache.txt >nul
if errorlevel 1 (
